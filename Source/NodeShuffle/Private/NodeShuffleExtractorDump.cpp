...
namespace
{
    // Packet G (ns-automatch): NodeShuffleFormName, FSfPlusAllowListReadout/ReadSfPlusAllowList, and
    // the node-group struct+census were factored out into NodeShuffleExtractorDiscovery.h/.cpp so
    // Packet G's auto-allow pass consumes the SAME data instead of a second copy. Those helpers are now
    // PURE (no logging inside) -- this command reconstructs the SAME log lines it always printed, right
    // after each call, so its own log output is unchanged. FNodeGroupInfo -> FNodeShuffleNodeGroupInfo
    // (renamed for the shared header; same fields).
...
static FAutoConsoleCommandWithWorldAndArgs GNodeShuffleDumpExtractorsCmd(
    ...
    FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& /*Args*/, UWorld* World)
    {
        ...
        // ---- 1. SF+ allow-list ----
        // ReadSfPlusAllowList() is now PURE (NodeShuffleExtractorDiscovery.cpp, Packet G) -- these three
        // log lines used to live inside it; reconstructed here verbatim so this command's own log
        // output is UNCHANGED (see the report's parity argument for the byte-for-byte comparison).
        const FSfPlusAllowListReadout AllowList = ReadSfPlusAllowList();
        static const TCHAR* SfPlusAssetPath = TEXT("/SatisfactoryPlus/AssetDatas/Modules/PDA_SFP_ExtractorList.PDA_SFP_ExtractorList");
        if (!AllowList.bAssetFound)
        {
            UE_LOG(LogNodeShuffle, Display,
                TEXT("DUMPEXTRACTORS: SF+ allow-list asset '%s' not found -- SF+ not installed (or its path changed). ")
                TEXT("Every extractor below will read sfPlusAllowList=<no-sfplus-installed>."), SfPlusAssetPath);
        }
        else if (!AllowList.bPropertyReadable)
        {
            UE_LOG(LogNodeShuffle, Warning,
                ...
                SfPlusAssetPath, *AllowList.PropertyInnerKind);
        }
        else
        {
            UE_LOG(LogNodeShuffle, Display,
                ...
                *AllowList.PropertyInnerKind, AllowList.ArrayEntryCount, AllowList.AllowedExtractorClasses.Num(),
                AllowList.UnresolvedEntries);
        }
        ...
        // CollectNodeGroups() is the shared, pure census (NodeShuffleExtractorDiscovery.cpp, Packet G) --
        // identical loop this command always ran, now reused by the auto-allow pass too.
        TMap<FString, FNodeShuffleNodeGroupInfo> NodeGroups;
        int32 TotalNodeInstances = 0;
        CollectNodeGroups(World, NodeGroups, TotalNodeInstances);
        UE_LOG(LogNodeShuffle, Display,
        ...
        for (const TPair<FString, FNodeShuffleNodeGroupInfo>& Pair : NodeGroups)
        {
            const FNodeShuffleNodeGroupInfo& G = Pair.Value;
            UE_LOG(LogNodeShuffle, Display,
