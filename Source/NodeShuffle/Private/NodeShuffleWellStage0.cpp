                        ...
                        int32 AttemptsToExhaust, int32 SatPlacementTries, double ElapsedMs)
{
    ...
    UE_LOG(LogNodeShuffle, Display,
        ...
        TEXT("measured in XY of the settled core -- a superset of every node that could have REJECTED ")
        TEXT("a candidate, and NOT a copy of the whole level; the per-probe walk saw more than this, ")
        TEXT("and nothing it saw beyond this radius can reject a candidate drawn inside the satellite ")
        TEXT("radius this scope is derived from. The same deposit exclusion, the same self-exclusion ")
        TEXT("and the same radius test then run per probe exactly as before. Nothing ")
